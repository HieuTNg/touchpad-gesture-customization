...
export class TwoFingerNavExtension implements ISubExtension {
    ...
    private _onCapturedEvent(
        ...
        if (type !== Clutter.EventType.SCROLL) return Clutter.EVENT_PROPAGATE;
        ...
        if (isNaN(dx) || isNaN(dy)) {
            log('[TwoFingerNav] IGNORED NaN DELTA');
            return Clutter.EVENT_PROPAGATE;
        }

        // Trace log to verify event interception
        log(`[TwoFingerNav] Scroll: dx=${dx.toFixed(2)}, dy=${dy.toFixed(2)}`);
